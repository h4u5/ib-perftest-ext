	...
	while (totscnt < tot_iters  || totccnt < tot_iters || (user_param->test_type == DURATION && user_param->state != END_STATE) ) {
		...
		for (index =0 ; index < num_of_qps ; index++) {
			...
			while ((ctx->scnt[index] < user_param->iters || user_param->test_type == DURATION) && (ctx->scnt[index] - ctx->ccnt[index]) < (user_param->tx_depth) &&
				...
				#else
					err = ibv_post_send(ctx->qp[index],&ctx->wr[index*user_param->post_list],&bad_wr);
          // call to get cycles to sleep for set time -- TLG
          if (user_param->usec_sleep){
            if (user_param->usec_sleep > 3000){
              usleep(user_param->usec_sleep);
            }
            // fine grained polling for high resolution delay
            else if (user_param->usec_sleep > 0){
              unsigned long long c1, c2, c3;
              c3 = user_param->cycles_sleep;
              c1 = get_cycles();
              do {
                 c2 = get_cycles(); 
              } while (c2-c1 < c3);
...
int run_iter_bw_infinitely(struct pingpong_context *ctx,struct perftest_parameters *user_param)
{
	...
	// main loop for posting
  // added in print - TLG
  //fprintf(stderr, "tx_depth: %li\n", user_param->tx_depth);
	while (1) {
		// main loop to run over all the qps and post each time n messages
		for (index =0 ; index < num_of_qps ; index++) {
      // by default, tx_depth seems to be 128
      // I need to check if we send 128 messages per ibv_post_send?
