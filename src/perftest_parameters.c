...
static void usage(const char *argv0,VerbType verb,TestType tst)	{
    ...
    // TLG
		printf("      --usec_sleep=<usecs> ");
		printf(" Run test with a <usecs> delay between each write.\n");
    
		printf("      --gap_survey=<usecs> ");
		printf(" Survey the impact of varying gap frim --usec_sleep to 1.\n");
...
static void init_perftest_params(struct perftest_parameters *user_param) {
  ...
  // Begin TLG params
  user_param->gap         = 0;
  user_param->usec_sleep  = 0;
  user_param->cycles_sleep= 0;
  user_param->gap_iter    = 1;
  user_param->gap_survey  = 0;
  // End TLG params
...
int parser(struct perftest_parameters *user_param,char *argv[], int argc) {
	...
	int size_factor = 1;
  //TLG
  static int gap_survey_flag = 0;
  static int usec_sleep_flag = 0;
	...
	while (1) {
      ...
      // added usec_sleep arg -- TLG
      { .name = "gap_survey",     .has_arg = 0, .flag = &gap_survey_flag, .val = 1 },
      { .name = "usec_sleep",     .has_arg = 1, .flag = &usec_sleep_flag, .val = 1 },
      ...
      // TLG
				if (usec_sleep_flag) {
					user_param->usec_sleep = strtol(optarg,NULL,0);
          user_param->cycles_sleep = user_param->usec_sleep * get_cpu_mhz(user_param->cpu_freq_f);
					usec_sleep_flag = 0;
				}
				if (gap_survey_flag) {
					user_param->gap_survey = 1;
					gap_survey_flag = 0;
          if (user_param->usec_sleep <= 0) {
            fprintf(stderr, "Cant survey impact of gap (delay) without --usec_sleep specified.\n");
            return FAILURE;
          }
				}
				if (pkey_flag) {
...
void print_report_bw (struct perftest_parameters *user_param, struct bw_report_data *my_bw_rep) {
  ...
  //calculate gap (us) and slowdown -- TLG
  //bw_avg is already in MB so we don't have to convert to us.

  if (user_param->gap_iter == 1){
    user_param->gap = user_param->size/bw_avg;
  }
  if (user_param->gap_survey){
    if (user_param->usec_sleep >= 1){
      fprintf(stdout, "Next iteration, sleeping %li\n", user_param->usec_sleep);
      user_param->usec_sleep = user_param->usec_sleep - 10;
    }
    else{
      exit(1);
    }
  }
  //get the number of cycles per microsecond -- TLG
  user_param->cycles_sleep = user_param->usec_sleep * get_cpu_mhz(user_param->cpu_freq_f);
